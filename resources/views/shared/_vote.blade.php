...
<div class="d-fex flex-column vote-controls">
    ...
    <form action="/{{ $firestURISegment }}/{{ $model->id }}/vote" id="up-vote-{{ $name }}-{{ $model->id }}"  method="POST" style="display: none">
        @csrf
    ...
    <form action="/{{ $firestURISegment }}/{{ $model->id }}/vote" id="down-vote-{{ $name }}-{{ $model->id }}"  method="POST" style="display: none">
        @csrf
        <input type="hidden" name="vote" value="-1">
    </form>
    @if($model instanceof App\Question)
